...
async def test_radius_miles_is_unconstrained_and_accepts_a_negative_value(
    db_session: AsyncSession,
    make_user: Callable[..., User],
    make_saved_search: Callable[..., SavedSearch],
) -> None:
    """Explicit product decision: unlike the integer numeric fields above,
    `radius_miles` has no non-negative CHECK. This proves that decision is
    actually implemented, not merely absent by omission."""
    user = await _insert_user(db_session, make_user, "radius-unconstrained@example.com")
    saved_search = make_saved_search(user.id)
    ...
    assert saved_search.radius_miles == Decimal(-5)
