...
| existing_value | jsonb, not null | for `evidence_mismatch`: the existing occurrence's disputed field value(s), e.g. `{"canonical_url_normalized": "..."}`. For `ambiguous_match`: the complete, deterministically sorted JSON array of every candidate `Job`'s UUID (as a string) that matched — never a truncated probe result (Phase 2's `ambiguous_match` conflict-persistence slice; no schema change — this column's own array-shape `CHECK` already permitted it). **Rev 18: `CHECK` enforces this shape by type** (object vs. array) — see below. Not `MutableDict`/`MutableList`-wrapped: an immutable, write-once snapshot, same rationale as `raw_job_ingestions.raw_payload` |
| incoming_value | jsonb, not null | the incoming payload's corresponding value(s) — same shape convention and `CHECK` as `existing_value`. For `ambiguous_match` specifically: a single-element JSON array holding the one newly created `JobOccurrence`'s UUID (as a string) — intentionally a different entity type than `existing_value`'s array of candidate `Job` IDs, not a symmetry bug |
